...
void requestQueue::requestQuery(QByteArray query) {
...
QByteArray requestQueue::getQueryKind() {
	if (isEmpty()) {
		return "";
	}
...
bool DB_MySQL::connectDB(void) {
	...
	qDebug() <<  "connecting... -> " << DB_HOST << ":" << DB_PORT;

	if (!(connection = mysql_real_connect(&mysql, DB_HOST, 
												 DB_USER, DB_PASS, DB_NAME, 
												 DB_PORT, NULL, 0))) {
		qDebug() <<"connecting Faild";
		return false;
	}
	else {
		qDebug() << "connecting Success";
		return true;
	}
}
...
bool DB_MySQL::updateQuery(const char *query) {
	...
	bool res = mysql_query(connection, query);
	if (res != 0) {
...
void DB_MySQL::requestQuery(QByteArray query) {
	...
	while (true) {
		if (mQueryQueue.isEmpty()) {
			...
			if (isReading) {
				...
				QByteArray strQuery = mQueryQueue.getStrQuery_popBack();
